...
> 对于包含且使用浮点单元的 CM4 会有 26 个寄存器压栈，本文只针对 CM3 描述。
...
> xPSR 寄存器的中的第 9 位不会被修改（存疑），修改的是栈帧中的 xPSR 值的第 9 位。
...
处理器在进入异常处理函数后，会设置 LR = EXC_RETURN 。当异常函数返回时会将 LR 加载到 PC ，硬件检测到 PC = EXC_RETURN 后会触发异常返回机制。
