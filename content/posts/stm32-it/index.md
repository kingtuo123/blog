...
> 另外，由于 CM3 支持 16 位和 32 位的混合指令（以下两种情况 PC 值不一定相等）：
>
...
||||
|:--|:--|:--|
|**普通函数调用** |编译器通过软件指令调整 SP  |编译器知道调用处的上下文，可以预判对齐需求    |
|**异常/中断**    |硬件自动双字（8字节）对齐  |异常随时可能发生，SP 状态不确定，必须硬件对齐 |
{{< /table >}}
