...
|                               |                     |                                  |
|:------------------------------|:--------------------|:---------------------------------|
|**传送数据（Move）**           |`MOV` `R4, R0`       |`R4` ← `R0`                       |
|                               |`MOVS` `R4, R0`      |`R4` ← `R0`，更新 `APSR`          |
...
- `LDR R0, =label` ： 
  - 通常被替换为 `LDR R0，[PC, #offset]` 。例如当 `lable` 的地址为 `0x12345678` 时，`0x12345678` 会被存入文字池，通过计算 `0x12345678` 所在地址相对于 PC 的偏移量，用 `LDR R0，[PC, #offset]` 替代
