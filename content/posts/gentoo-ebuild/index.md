...
在 EAPI 8 中，可以通过在地址前添加 `mirror+` 或 `fetch+` 来使 `SRC_URI` 中的单个项目免于自动镜像和下载限制：
...
|                    |无前缀          |fetch+          |mirror+         |
|:-------------------|:---------------|:---------------|:---------------|
|不使用 `RESTRICT`   |自动镜像 + 下载 |自动镜像 + 下载 |自动镜像 + 下载 |
|`RESTRICT="mirror"` |下载            |下载            |自动镜像 + 下载 |
|`RESTRICT="fetch"`  |X               |下载            |自动镜像 + 下载 |
{{< /table >}}


> `RESTRICT="fetch"` &ensp;表示禁止下载文件，用户需要手动下载源文件后放到 `/var/cache/distfiles` 目录。
> 
> `RESTRICT="mirror"` 表示禁止自动镜像（不从镜像站获取文件），即直接使用 ebuild 中的原始 URI 获取文件。
...
参考 [Gentoo 中文社区 / 镜像列表](https://www.gentoo.org.cn/mirrorlist/) 。
...
- 配置文件：
  - `/etc/portage/make.conf` 中 `GENTOO_MIRRORS` 用于下载**源代码**（ebuild 中 SRC_URI 的文件）和 **Portage 快照**（`emerge-webrsync`）。
  - `/etc/portage/repos.conf/gentoo.conf` 中 `sync-uri` 用于**同步 Portage 树**（即同步 ebuild 文件，`emerge --sync`）。
...
```bash{ bar="/var/db/repos/gentoo/acct-group/docker/docker-0-r3.ebuild" }
# Copyright 1999-2024 Gentoo Authors
# Distributed under the terms of the GNU General Public License v2
...
```bash{ bar="/var/db/repos/gentoo/acct-user/docker_auth/docker_auth-0-r3.ebuild" }
...
```bash{ bar="/var/db/repos/localrepo/metadata/layout.conf" }
...
```bash{ bar="/etc/portage/repos.conf/localrepo.conf" }
[localrepo]
...
需要先安装 `app-eselect/eselect-repository` 这个模块，然后按如下命令执行：
...
```bash{ bar="/etc/portage/repos.conf/eselect-repo.conf" }
[testrepo]
...
```bash{ bar="/var/db/repos/localrepo/app-misc/hello/hello-1.0.ebuild" }
# Copyright 1999-2024 Gentoo Authors
# Distributed under the terms of the GNU General Public License v2
